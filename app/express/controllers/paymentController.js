...
module.exports = function paymentController(app) {
  ...
  async function create(req, res) {
    const createdPayment = await app.payments.create(req.body, req, req.files);
  ...
  /** @api {get} /api/v1/payments/:slug
   * @apiName getById
   * @apiGroup payment
   *
   */
  async function getBySlug(req, res) {
    const response = await app.payments.getBySlug(req.params.slug);
    res.json(response);
  }
